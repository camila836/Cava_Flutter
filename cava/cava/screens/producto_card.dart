...
class ProductoDestacadoCard extends StatelessWidget {
  const ProductoDestacadoCard({super.key, required this.producto, this.onVerProducto});
  final Producto producto;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        AspectRatio(
          ...
          child: producto.imagenUrl == null
              ? const _Placeholder()
              : Image.network(producto.imagenUrl!, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const _Placeholder()),
        ),
        Padding(
          padding: const EdgeInsets.fromLTRB(12, 10, 12, 12),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(producto.nombre, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: CavaColors.ink)),
            const SizedBox(height: 4),
            Text(precio, style: const TextStyle(fontSize: 13, color: CavaColors.wine, fontWeight: FontWeight.w700)),
