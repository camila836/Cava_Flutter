  ...
  Future<void> _registrar() async {
    ...
    if (_fechaNacimiento == null || !_autoriza) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_fechaNacimiento == null
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(session.error ?? 'No se pudo completar el registro.')));
    }
  }
          ...
          child: Form(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                        'Completa tus datos para disfrutar nuestra selección artesanal.'),
                    const SizedBox(height: 25),
                    _campo(_nombres, 'Nombres', 'Tu nombre',
                        validator: _requerido),
                    _campo(_apellidos, 'Apellidos', 'Tus apellidos',
                        validator: _requerido),
                    DropdownButtonFormField<int>(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: 4, child: Text('Cédula de ciudadanía')),
                        DropdownMenuItem(
                            value: 5, child: Text('Cédula de extranjería')),
                        DropdownMenuItem(value: 7, child: Text('Pasaporte')),
                        DropdownMenuItem(
                            value: 8,
                            child: Text('Permiso por Protección Temporal')),
                        DropdownMenuItem(
                            value: 6, child: Text('Tarjeta de identidad')),
                      ],
                      onChanged: (v) =>
                          setState(() => _idTipoDocumento = v ?? 4),
                    ),
                    const SizedBox(height: 13),
                    _campo(_identificacion, 'Identificación',
                        'Número de documento',
                        keyboard: TextInputType.number, validator: _requerido),
                    _campo(_correo, 'Correo electrónico', 'correo@ejemplo.com',
                    ...
                    _campo(_direccion, 'Dirección', 'Dirección de residencia',
                        validator: _requerido),
                    _campo(_telefono, 'Teléfono', '+573001234567',
                        keyboard: TextInputType.phone, validator: _requerido),
                    InkWell(
                    ...
                    const SizedBox(height: 13),
                    _campo(_password, 'Contraseña', 'Mínimo 8 caracteres',
                        obscure: _ocultar,
                        suffix: IconButton(
